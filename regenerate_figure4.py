"""Regenerate figure4.pdf (main text, fig:tightness) at a shorter height.

Same pipeline as regenerate_figure5.py (that one is N=3072 CIFAR-10 32x32;
this one is the original N=4 grayscale-2x2-downsampled CIFAR-10 block from
figures.py), so the underlying numbers are unchanged. Main-text figure, so
less aggressive than the appendix cut: 5.5x3.09in -> 5.5x2.3in (vs 1.75in
for Fig 5), plus sharing the x-axis across the MSE/PSNR rows so tick labels
aren't drawn twice.
"""
import pickle
...
# --- exact data generation from figures.py (N=4 grayscale-2x2 CIFAR-10 block) ---
CACHE = Path(__file__).parent / "figure4_recon_scores.pkl"
if CACHE.exists():
...
fig, axs = plt.subplots(2, 3, figsize=(5.5, 1.8), sharex=True, sharey="row", layout="constrained")
